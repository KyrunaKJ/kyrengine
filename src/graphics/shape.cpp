...
void Shape::load_texture_vertices_from_file(const string& file_path, AppConfig& config) {
    ifstream file(file_path);
    if (!file.is_open()) {
        throw runtime_error("Failed to open file: " + file_path);
    }
    ...
    while (getline(file, line)) {
        ...
        if (iss >> x >> y) {                       
            vertices.uv.push_back(x);
            vertices.uv.push_back(y);
        }
    }
...
void Shape::create_vbo() {
    ...
    glBufferData(GL_ARRAY_BUFFER, vertices.position.size() * sizeof(float), vertices.position.data(), GL_STATIC_DRAW);
